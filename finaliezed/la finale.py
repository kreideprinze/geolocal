...
for pkg in ['cv2', 'numpy', 'pymavlink']:
    try:
        __import__(pkg)
    except:
...
def find_cam_to_drone_transform(filename="cam_to_drone_transform.csv"):
    for root, _, files in os.walk("/"):
        if filename in files:
            return os.path.join(root, filename)
    raise FileNotFoundError(f"{filename} not found in filesystem")

def load_cam_to_drone_transform():
    path = find_cam_to_drone_transform()
...
state = {'lat': None, 'lon': None, 'alt': None, 'roll': None, 'pitch': None, 'yaw': None, 'baro_alt': None}
...
idx = 0
while os.path.exists(f"output_{idx}.avi"):
    idx += 1
video_writer = cv2.VideoWriter(f'output_{idx}.avi', fourcc, 20.0, (frame_width, frame_height))
...
try:
    while True:
        ...
        if ids is not None:
            rvecs, tvecs, _ = aruco.estimatePoseSingleMarkers(corners, 0.5, camera_matrix, dist_coeffs)
            for i in range(len(ids)):
                ...
                if None not in (lat, lon, alt, roll, pitch, yaw):
                    ...
                    m_id = ids[i][0]
                    marker_data[m_id].append((m_lat, m_lon, m_alt, dist, frame_idx, frame.copy()))
        ...
        if None not in (roll, pitch, yaw):
            ...
            Rz = np.array([[np.cos(yaw),-np.sin(yaw),0],[np.sin(yaw),np.cos(yaw),0],[0,0,1]])
            R = Rz @ Ry @ Rx
            axes = {(0,0,255): R @ np.array([1,0,0])*L,(0,255,0): R @ np.array([0,1,0])*L,(255,0,0): R @ np.array([0,0,1])*L}
